...
export async function middleware(request: NextRequest) {
  ...
  if (token && token.role) {
    ...
    // Basic role check - only admin users can access admin app
    if (userRole !== 'admin') {
      // Redirect non-admin users to store (temporary solution)
      console.log(`🚫 ${userRole} user accessing admin app, redirecting to store`)
