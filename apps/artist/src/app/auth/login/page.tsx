...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.user && !isRedirecting) {
      ...
      // Redirect to dashboard for artists
      router.replace(next);
    }
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      const result = await signIn('credentials', {
        ...
        appName: 'artist', // Specify this is for artist app
        redirect: false,
      });
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-gray-600">
            Access your artist dashboard and portfolio
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="mt-6 bg-gray-50 p-4 rounded-md">
              <h3 className="text-sm font-medium text-gray-900 mb-2">Test Artist Account:</h3>
              <div className="text-xs text-gray-600 space-y-1">
                <div><strong>Email:</strong> artist1@artistryhub.com</div>
                <div><strong>Password:</strong> Artist2024!Creative#</div>
              </div>
            </div>
          </form>
        </CardContent>
