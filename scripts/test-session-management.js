...
async function testSessionManagement() {
  ...
  try {
    for (const scenario of testScenarios) {
      ...
      if (!isValidPassword) {
        console.log(
          `   ❌ PASSWORD VERIFICATION FAILED: ${scenario.user.email}`
        );
        continue;
      }
      ...
      if (dbUser.role !== scenario.user.role) {
        console.log(
          `   ❌ ROLE MISMATCH: Expected ${scenario.user.role}, got ${dbUser.role}`
        );
        continue;
      }
      ...
      Object.entries(scenario.expectedAccess).forEach(
        ([app, expectedAccess]) => {
          const hasAccess = scenario.expectedAccess[app];
          const status = hasAccess === expectedAccess ? "✅" : "❌";
          console.log(
            `      ${status} ${app}: ${hasAccess ? "ACCESS" : "DENIED"}`
          );
        }
      );
    ...
    for (const test of crossAppTests) {
      ...
      console.log(
        `   ${status} Test Result: ${shouldRedirect ? "Redirect" : "No Redirect"}`
      );
      console.log("");
    }
