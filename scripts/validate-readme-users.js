...
async function validateUsers() {
  ...
  try {
    ...
    for (const expectedUser of expectedUsers) {
      ...
      // Validate password
      const isValidPassword = await bcrypt.compare(expectedUser.password, dbUser.hashedPassword);
      
      if (!isValidPassword) {
        console.log(`❌ PASSWORD MISMATCH: ${expectedUser.email} - Expected: ${expectedUser.password}`);
        validationErrors++;
        continue;
      }
      ...
      if (dbUser.role !== expectedUser.role) {
        console.log(`❌ ROLE MISMATCH: ${expectedUser.email} - Expected: ${expectedUser.role}, Got: ${dbUser.role}`);
        validationErrors++;
        continue;
      }
      ...
      if (dbUser.status !== 'ACTIVE') {
        console.log(`❌ STATUS MISMATCH: ${expectedUser.email} - Expected: ACTIVE, Got: ${dbUser.status}`);
        validationErrors++;
        continue;
      }

      console.log(`✅ VALID: ${expectedUser.email} (${expectedUser.role}) - ID: ${dbUser.id}`);
      validationSuccess++;
    }
